...
export default function CustomerView() {
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 pb-9 bg-white rounded-[40px] shadow-lg shadow-black/25">
      ...
      <p className="mb-6 text-center font-medium">Host: {status.queue.businessName}</p>
      ...
      <div className={`p-4 border rounded-3xl mb-6 ${getStatusColor()}`}>
        ...
        {status.status !== 'WAITING' && status.status !== 'CALLED' && (
          <div className="text-center font-semibold">{getStatusDisplay()}</div>
        )}
      </div>

      <div className="mb-4 text-xs text-center text-gray-900">
        <p>
          You {status.status === 'CANCELLED' ? 'left' : 'joined'} this queue on{' '}
          {new Date(status.status === 'CANCELLED' ? new Date() : status.joinedAt).toLocaleString()}
        </p>
      </div>
        ...
        {status.status === 'CANCELLED' ? (
          ...
          <>
            <p className="text-xs text-gray-900 mb-3">
              Keep this page open to maintain your position and receive notifications when it's your
              turn.
            </p>
