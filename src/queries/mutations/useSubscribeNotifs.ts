...
async function subscribeWithRetry(
  ...
  try {
    ...
    if (isRetryableError && retryCount < MAX_RETRIES) {
      ...
      console.error(
        `Retrying subscription (attempt ${retryCount + 1}/${MAX_RETRIES}) in ${delay}ms...`,
      )
...
export function useSubscribeNotifs() {
  return useMutation({
    mutationFn: async (params: SubscribeNotificationParams) => subscribeWithRetry(params),
    retry: false,
    ...
    onSuccess: () => {
      console.info('Successfully subscribed to notifications')
    },
  })
}
